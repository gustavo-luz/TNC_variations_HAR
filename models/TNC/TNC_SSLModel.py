...
class TNC(BaseModelClass):
    def __init__(
        ...
        if config.encoder_type == 'TS2Vec':
            self.discriminator = Discriminator_TS2Vec(config.encoder_dims).to(self.device)
        elif config.encoder_type == 'RNN':
            self.discriminator = Discriminator_RNN(config.encoder_dims).to(self.device)
...
class Discriminator_TS2Vec(torch.nn.Module):
    def __init__(self, input_size):
        super(Discriminator_TS2Vec, self).__init__()
...
# disc without max pooling - for rnn
...
class TNCDataset(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        # end = time.time()
        # print(f"time to run neighbourhood:{end-start:.3f}")
    ...
    def _find_neighours(self, x, t):
        ...
        if self.adf:
            ...
            # target_window is the segment of the time series centered at time t with length equal to window_size. 
            # The window is then flattened into a one-dimensional array to facilitate cosine similarity calculations.
            ...
            # Sort by similarity and take top `mc_sample_size`
